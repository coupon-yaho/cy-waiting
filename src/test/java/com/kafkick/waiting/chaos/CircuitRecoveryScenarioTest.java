...
class CircuitRecoveryScenarioTest {
    ...
    void C8_서킷이_열렸다가_두_번_안에_닫힌다() {
        ...
        ChaosScenario.named("C8 뒷단 지연 → 서킷 오픈")
                ...
                //
                // **"회복 시도 ≤ 2" 는 여기서 안 잰다.** 두드리는 루프가 열린 상태를
                // 벗어나는 순간 빠져나와 관측값이 늘 1 이었다. 합성 프로브를 켜고
                // half-open 진입을 세는 C8c 가 잰다 (CY-834).
                //
                // **지연 갈래도 안 밟는다.** 응답 상한이 느린 호출 임계보다 낮아
                // 모든 호출이 타임아웃으로 끊긴다. 느린 호출로 여는 것은 C8b 가 잰다.
                .run();
    }
    ...
    // **G8.12 판정을 안 건다.** 두드리는 루프로는 관측값이 늘 1 이라 "2 이하" 가
    // 항진명제였다. 합성 프로브로 half-open 진입을 세고, 안 살린 뒷단에서 한계를
    // 넘는 대조 판까지 두는 C8c 가 잰다 (CY-834).
}
