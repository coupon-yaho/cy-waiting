...
class MixedVersionSnapshotScenarioTest {
    ...
    /**
     * 옛 판이 이 해시를 읽으면 쿠폰을 제대로 받는가.
     *
     * <p><b>자리 수만 세면 항등이다.</b> 새 판의 부호화는 늘 여섯 조각이라 무엇을
    ...
    private long 옛_판이_읽는_재고(Map<String, String> hash) {
    ...
    void C20_옛_판과_새_판이_서로의_스냅샷을_받아들인다() {
        ...
        boolean[] 옛_판이_발행으로_읽히는가 = new boolean[1];
        boolean[] 자기_판이_발행으로_읽히는가 = new boolean[1];
        ...
        ChaosScenario.named("C20 혼재 버전 스냅샷")
                ...
                .assertEntry(() -> RecoveryCriteria.violations(
                        ...
                        쿠폰을_받았다("자기 판", 자기가_낸_것[0]),
                        발행으로_읽힌다("자기 판", 자기_판이_발행으로_읽히는가[0]),
                ...
                // **일부러 안 잰다.** 이 판에는 걷을 장애가 없다 — 서로 다른 입력을
