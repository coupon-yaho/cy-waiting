...
printf '  %-24s %s%%\n' "판정한 요청 비율" "$pct"
printf '  %-24s %s%%\n' "기준" "$target"
# 이 회차가 몇 건까지 허용하는지 같이 낸다. 백분율만 찍으면 낡음 창 하나가
# 예산을 통째로 먹는다는 사실이 뒤에 숨는다 (O-10).
# **판정 술어에서 직접 뽑는다.** 대수적으로 옮긴 식은 판정과 다른 반올림을 타
# 표시와 판정이 갈린다 — `100 - 99.9` 가 0.09999999999999432 라 그냥 자르면 20 이
# 19 가 되고, 보정을 넣으면 목표의 소수 자리가 넉넉할 때 반대로 어긋난다.
# 여기서는 "그만큼 열화해도 충족인 가장 큰 수" 를 술어로 찾으므로 정의상 같다.
budget=$(awk -v n="$total" -v t="$target" '
    BEGIN {
        b = int(n * (100 - t) / 100)
        while (b > 0 && (n - b) / n * 100 < t) { b-- }
        while ((n - (b + 1)) / n * 100 >= t) { b++ }
        printf "%d", b
    }')
printf '  %-24s %s건\n' "이 회차의 열화 예산" "$budget"
