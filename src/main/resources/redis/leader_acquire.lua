...
-- 평활화 상태가 초기화된다 (F9).
--
-- **펜스 번호를 같이 담는다** (CY-766). 되돌릴 수 없는 쓰기는 이 번호를 들고
...
if owner == ARGV[1] then
    ...
    -- **펜스 번호는 그대로 둔다.** 매 틱 새로 매기면 자기 자신을 옛 리더로 만든다.

    -- **번호가 없으면 그때는 매긴다.** 옛 형식으로 잡아 둔 락을 물려받으면 번호가 0 인데,
    -- 0 은 울타리가 전부 거절하는 값이라 그 노드의 매진 큐 정리가 리스가 끊길 때까지
    -- 죽는다 — 연장이 번호를 안 바꾸므로 스스로 못 빠져나온다.
    if fence <= 0 then
        local t = redis.call('TIME')
