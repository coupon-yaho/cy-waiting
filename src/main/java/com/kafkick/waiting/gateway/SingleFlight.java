...
/**
 * 같은 키로 동시에 온 조회를 <b>뒷단 한 번</b>으로 모은다. 발급은 판정이 막지만 조회는
 * 그대로 통과해, 오픈 순간 쿠폰 페이지 열람이 뒷단에 전량 꽂힌다.
 *
 * @param <T> 뒷단이 돌려주는 것
 */
public final class SingleFlight<T> {
    ...
    /**
     * 도는 것이 있으면 거기 붙고, 없으면 시작한다. 뒷단 호출을 구독자와 떼어 놓는 것은,
     * 붙어 있으면 첫 구독자가 취소할 때 뒤엣사람 전부가 빈손이 되기 때문이다.
     *
     * @param call 도는 것이 없을 때 부를 것. 있으면 <b>안 부른다</b>
     */
    public Mono<T> join(String key, Supplier<Mono<T>> call) {
        ...
        synchronized (this) {
            ...
            if (running == null) {
                // 상한을 넘으면 모으지 않고 그냥 보낸다. 여기서 거절하면 보호 장치가
                // 조회를 끊는 것이 되어, 없느니만 못하다.
    ...
    /**
     * 구독자와 뗀 뒷단 호출. {@code cache()} 가 결과를 여럿에게 나눠 주고,
     * {@code publish().refCount()} 는 안 쓴다 — 그쪽은 구독자가 0 이 되면 원본을 끊는다.
     */
    ...
    /** 지금 모으고 있는 키 수. 지표가 이 값을 읽는다. */
    public synchronized int inFlight() {
        return flights.size();
    }

    /**
     * 상한에 닿았는가. <b>닿으면 모으기가 조용히 멎는다</b> — 뒷단 도달 수만 원상복귀하고
     * 지표에는 아무것도 안 남으므로, 부르는 쪽이 그 사실을 남긴다.
     */
