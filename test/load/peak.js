...
// **폴링으로 뽑히고도 폴링을 못 한 회차의 비율.** 분모는 전체 요청이 아니라
// 폴링으로 뽑힌 회차다 — 그 갈래가 얼마나 제 일을 했는지를 재는 자리라 그렇다.
// 그 갈래는 폴링 대신 진입을 보내므로 그만큼 폴링이 10% 아래로, 진입이 30% 위로
// 간다. 세기만 하면 몫이 어긋난 회차가 그대로 지나간다.
...
export const options = {
  ...
  thresholds: {
    ...
    checks: ['rate>0.99'],
    // **폴링으로 뽑힌 회차 중** 표가 없어 진입으로 돌린 몫. 모든 VU 가 표 없이
    // 시작하므로 0 은 될 수 없다. 전체 요청 기준으로는 그 10 분의 1 이므로,
    // 이 임계는 폴링 몫이 10% 에서 9% 아래로 내려가는 것을 막는다.
