...
--
-- **돌아와도 순번은 안 돌려준다** (D-11). 비운 사이에 온 사람을 뒤로 밀면 그건
-- 추월이다. rank 를 여기서 함께 내는 것도 같은 이유다 — 따로 물으면 그 사이 앞사람이
-- 빠져 자기 순번보다 작은 수를 받고, 사용자가 보기엔 줄이 뒤로 간 것이다.
--
-- 순번이 카운터가 아니라 **벽시계**다 (A-9). 시계가 뒤로 가면 나중에 온 사람이 앞서
-- 불변식 4 가 깨지므로 maxscore 로 바닥을 깐다. ZSET 의 마지막 원소로는 **큐가 빈
...
-- **복제 단위로는 함께 움직인다.** Lua 는 효과 기반 복제라 이 스크립트가 남긴 쓰기는
-- 복제본과 AOF 에 통째로 가거나 통째로 안 간다 — maxscore 가 ZSET 보다 뒤처진 채
-- 복제되는 상태는 없다.

-- **다만 스크립트 안의 롤백은 없다.** 그래서 실패할 수 있는 인자 검증을 전부 위로
-- 올려 뒀다 — 여기 도달하면 남는 실패 경로는 메모리 부족뿐이고 maxmemory 로 막는다.
redis.call('ZADD', KEYS[1], score, ARGV[1])
redis.call('SET', KEYS[2], score, 'EX', scoreTtl)
redis.call('ZADD', KEYS[3], now + aliveTtl, ARGV[1])

-- **tostring 을 쓰지 않는다.** Lua 5.1 은 수를 %.14g 로 문자열화하는데 마이크로초
-- score 는 16자리라 과학 표기로 접힌다. ZSET 에는 정확한 값이 들어가므로 내림 쪽으로
-- 접히면 앞사람보다 작은 score 를 쥐고 추월한다 (불변식 4).

-- %d 가 아니라 %.0f 다. %d 는 정수로 캐스팅해 32비트 런타임에서 넘치고, %.0f 는
-- 2^53 아래에서 정확하며 지수도 안 붙는다. 그리고 **개수를 세지 순위를 저장하지
-- 않는다** — 저장하면 앞사람이 빠질 때마다 전원을 갱신해야 한다.
local rank = redis.call('ZCOUNT', KEYS[1], '-inf', '(' .. string.format('%.0f', score))
-- **이탈 기록을 소비한다** (7.5.2). 안 지우면 다음에 또 재방문으로 나오고, 유예
-- 만료를 기다려야 정상으로 돌아온다. **입장 표시는 안 건드린다** — 같은 해시에
-- 종류가 둘이고, `a:` 를 지우면 입장 복구가 통째로 없어진다.

-- **보관 기간을 여기서도 잰다.** 스위퍼가 안 도는 구간(매진 중·장애 중, 줄이 빈
-- 쿠폰)에서는 낡은 기록이 남아 몇 시간 전에 떠난 사람이 재방문자로 나온다. 낡았어도
-- 지우기는 한다 — 안 지우면 그 사람이 다음에도 같은 판정을 받는다.
local rejoined = 0
