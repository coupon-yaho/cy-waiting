...
/**
 * 뒷단 서킷을 <b>인스턴스별로</b> 만든다. 전체를 하나로 묶으면 한 대가 죽어도 전
 * 트래픽이 막힌다. 그래서 판정이 보는 값도 "서킷이 열렸는가" 가 아니라 "살아 있는
 * 인스턴스 비율" 이다.
 */
@Configuration
...
public class BackendCircuit {
    ...
    public static CircuitBreakerRegistry registry(BackendCircuitProperties props) {
        ...
        return CircuitBreakerRegistry.of(CircuitBreakerConfig.custom()
                ...
                // **스스로 반쯤 연다.** 수동 전환은 호출이 와야 상태를 다시 보는데, 판정이
                // OPEN 에서 유효 credit 을 조이면 서킷에 닿는 호출이 0 이라 안 풀린다.
                // 가는 곳은 CLOSED 가 아니라 HALF_OPEN 이라 약한 뒷단에 전량이 안 꽂힌다.
                .automaticTransitionFromOpenToHalfOpenEnabled(true)
                .build());
    }

    /**
     * <b>전이를 진입·해제 쌍으로 로깅한다.</b> 판정이 OPEN 에서 유효 credit 을 조이면
    ...
    public CircuitBreakerRegistry backendCircuitRegistry(BackendCircuitProperties props,
        ...
        // **직접 묶는다.** 레지스트리를 손으로 만들면 자동 구성이 묶어 주던 계량기가
        // 빠진다. 라이브러리가 클래스패스에 있어도 그렇고, 지표를 실제로 긁어 보기
        // 전까지 안 드러난다 — 서킷이 열려도 알람과 대시보드는 조용하다.
        TaggedCircuitBreakerMetrics.ofCircuitBreakerRegistry(registry).bindTo(meters);
        return registry;
    }

    /**
     * 서킷 상태를 판정과 배분이 함께 읽는 자리. <b>하나를 나눠 쓴다</b> — 각자 만들면
     * 판정은 열렸다고 보는데 배분은 아니라고 보는 구간이 생기고, 그 어긋남이 하필 회복
     * 구간에 난다.
     */
    @Bean
    public CircuitStateReader circuitStateReader(CircuitBreakerRegistry circuits,
        ...
        // 내므로, 배선이 빠지면 서킷 연동이 통째로 꺼진 채 조용히 돈다.
