...
/**
 * 폴링 부하를 <b>큐를 훑지 않고</b> 구한다. 2만 명을 세면 그 계산 자체가 부하다.
 * 배수율을 알면 각 밴드 인원은 곱셈 몇 번으로 나온다 — 앞에서
 * {@code drainRate × 밴드폭} 명씩 채워 나간다.
 */
public final class PollBudgetPlanner {
    ...
    /**
     * 이 큐가 만드는 초당 폴링 수. {@code drainRate} 가 0 이면 전원의 ETA 가
     * 무한이라 가장 먼 밴드다 — 방어가 없으면 0 으로 나눠 터진다.
     */
    ...
    /**
     * 이 규모에서 감당할 폴링(초당). <b>상수 하나로 고정하면 증설이 폴링 간격을
     * 못 줄인다.</b> 0 이하 방어는 {@code SnapshotMeta.effectiveGatewayCount()} 하나가
     * 쥔다 — 사본이 생기면 한쪽을 지워도 다른 쪽이 막아 뮤테이션이 살아남는다.
     */
    ...
    /**
     * 이 재료로 걸리는 전역 폴링 배수. <b>조립을 한 자리에 둔다</b> — 부르는 쪽이
     * 저마다 조립하면 분모를 바꾸는 날 한쪽만 따라가고, 시나리오가 낡은 값을
     * 초록으로 단언한다.
     */
    ...
    /**
     * 한 틱의 폴링 예산과 그 결과. <b>직접 만들어도 값이 서로 맞아야 한다</b> —
     * 표준 생성자가 열려 있어, 배수만 1 로 박은 Scale 을 받으면 그 자리가 곧
     * "재는 척하는 픽스처" 가 된다.
     */
    ...
    /**
     * 예산을 넘은 비율. 이 배수만큼 모두의 간격을 함께 늘린다. <b>1 미만으로 안
     * 내려간다</b> — 한산하다고 더 자주 두드리게 하면 없던 부하를 만든다.
     */
