...
class NodeHalvingUnderScaleScenarioTest {
     ...
     *
     * <p>전원이 첫 밴드라 간격이 곧 배수다. 밴드가 하나뿐인 회차를 고른 것은
     * 천장이 안 물게 하기 위해서다 — 천장의 몫은 C19 가 잰다.
     */
    // **배수는 프로덕션이 조립하는 자리에서 받는다.** 여기서 예산과 배수를 다시
    // 조립하면, 어느 분모로 예산을 잡는지가 바뀌는 날(CY-732) 이 시험만 낡은
    // 값을 초록으로 단언한다.
    ...
    void C21_배수가_걸린_채_노드_절반이_사라지면_남은_노드가_설계값을_넘게_받는다() {
        ...
        ChaosScenario.named("C21 배수가 걸린 채 노드 절반 소실")
                ...
                .duringFault(() -> {
                    ...
                    실패가_연속을_끊는_회차(실패_끊긴_분모, 실패_뒤_분모);
                })
                .recover(() -> {
                    분모.observed(처음_노드);
                    회복_분모[0] = 분모.count();
                ...
                .assertEntry(() -> RecoveryCriteria.violations(
                        ...
                        // 동작을 실패로 잡는다. 이 회차가 성립하는 배포인지 먼저 본다.
                        하강_지연이_둘_이상이다(),
                        // 전제 — 밴드표가 이 줄을 전원 첫 밴드로 놓는다. 이것이
                        // 깨지면 아래 노드당 값이 다른 회차의 값이 된다.
                ...
                .assertDuring(() -> RecoveryCriteria.violations(
                        ...
                        // 관측 실패는 "더 작게 봤다" 가 아니다. 노드가 절반
                        // 죽는 상황이면 하트비트도 같이 실패한다.
                ...
                // **RC1~RC6 은 여기서 안 잰다.** 이 회차는 분모와 예산 계산만 걷는다.
    ...
    /**
     * 하강 연속 도중 관측이 한 번 실패하는 회차.
     *
     * <p>본 경로와 섞으면 두 가지를 한 분모에서 재게 되어 어느 쪽이 깨졌는지
     * 안 보인다. 같은 배선의 분모를 따로 세운다.
     */
    private void 실패가_연속을_끊는_회차(int[] 실패_끊긴_분모, int[] 실패_뒤_분모) {
        GatewayRegistry 섞인 =
                GatewayRegistry.of(배선.rampDownTicks(), 배선.expectedNodes());
    ...
    private Optional<String> 하강_지연이_둘_이상이다() {
        return 하강_지연_틱 >= 2 ? Optional.empty()
                : Optional.of("전제 — 하강 지연이 %d 틱이다. 이 회차는 2 틱 이상인 배포에서만 "
