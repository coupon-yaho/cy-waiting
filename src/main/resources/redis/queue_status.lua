...
if not score then
    -- **차례가 왔던 사람인지 먼저 본다.** 입장하면 큐에서 빼므로 다음 폴링은 줄에 없는
    -- 것으로 보인다. 그대로 두면 자기 차례를 받은 사람이 1초 뒤에 "매진" 을 보고, 다시
    -- 서면 그동안 온 사람들 뒤로 간다 (불변식 4). 탭이 둘이어도 같은 일이 난다.

    -- **종류를 보고 읽는다.** 이 해시에는 청소도 쓴다. 값 전체를 비교하면 시각이 붙은
    -- 순간 못 알아보고, 차례가 왔던 사람이 종료를 받는다.
