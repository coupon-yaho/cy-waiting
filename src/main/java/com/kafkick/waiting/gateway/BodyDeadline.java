...
/**
 * 본문이 <b>끝없이 느린</b> 뒷단을 끊는다. 응답 상한은 헤더까지만 재므로 본문을 한 바이트도
 * 안 보내도 안 걸린다 — 실측으로 40초를 기다려도 안 끝났다. <b>서킷 안쪽에는 못 둔다</b> —
 * 응답을 감싸는 자리가 서킷보다 바깥이라 여기서 끊은 것은 서킷 창에 안 쌓인다.
 */
public final class BodyDeadline implements GatewayFilter {
    ...
    private BodyDeadline(Duration limit, MeterRegistry meters) {
        ...
        // **0 이면 끊는 것이 아니다.** 값으로 끄면 그 사실이 설정 어디에도
        // 안 드러나고, 커넥션이 붙잡히는 것이 정상으로 읽힌다.
    ...
    /**
     * 응답 상한과의 앞뒤를 <b>기동에서</b> 못 박는다. 시험으로만 두면 배포 설정 한 줄이
     * 순서를 뒤집고, 짧으면 정상 속도로 흐르는 큰 응답을 이쪽이 먼저 죽인다. 격벽 시한과는
     * 겨루지 않는다 — 실측에서 자리는 15초에 돌아왔고 커넥션은 24.5초에 끊겼다.
     */
