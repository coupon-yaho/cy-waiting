...
**공개 저장소다.** 지금 첫 화면은 빈 파일 목록이라 처음 온 사람이 무엇인지 알
방법이 없다.
...
**라이선스·기여 정책·이슈 트래커는 PoC 범위 밖이다.** 셋 다 *밖에서 오는
사람*을 전제로 하는데, PoC 단계에서는 그런 사람이 없다. 없는 독자를 위해
문서를 쓰면 유지 대상만 늘고, 정작 정할 때가 오면 그때의 조건으로 다시
정하게 된다. **공개 시점에 정한다.**
...
| G1.15 | 루트 `README.md` 가 있다 | T1.5.1 |
