...
class ClockBackAndGhostScenarioTest {
     ...
     * 램프값과 같은 회차에 실리는 것</b>이 이 시나리오가 재려는 것이라, 그 회차를
     * 회복 안에서 실제로 만든다.
     */
    @Test
    ...
    void C17_시계가_뒤로_가도_크레딧이_유지되고_유령은_창_안에서만_세어진다() {
        ...
        ChaosScenario.named("C17 시계 역행과 유령 인스턴스")
                ...
                .afterRecovery(() -> {
                    ...
                    CapacityCollector 유령_회차 = 수집기();
                    유령_회차.collect(보고("유령", 마지막_보고), NOW, 노드);
                    유령[0] = 유령_회차.collect(보고("유령", 마지막_보고), 창_끝, 노드);
                    유령[1] = 유령_회차.collect(보고("유령", 마지막_보고), 창_끝 + 1, 노드);
                    유령[2] = 수집기().collect(보고("먼_미래", NOW + 앞섬_허용 + 1), NOW, 노드);

                    // 죽은 유령과 갓 뜬 뒷단이 한 회차에 같이 실린다.
                    CapacityCollector 겹침 = 수집기();
                    겹침.collect(보고("유령", 마지막_보고), NOW, 노드);
                    겹친_회차[0] = 겹침.collect(
                            List.of(하나("유령", 마지막_보고), 하나("갓_뜬", NOW + 1)),
                            NOW + 1, 노드);
                    // 같은 회차에서 유령만 뺀다. 차이가 곧 죽은 몫이다.
                    CapacityCollector 유령_없는_회차 = 수집기();
                    유령_없는_회차.collect(보고("유령", 마지막_보고), NOW, 노드);
                    겹친_회차[1] = 유령_없는_회차.collect(보고("갓_뜬", NOW + 1), NOW + 1, 노드);
                ...
                .assertRecovery(() -> RecoveryCriteria.violations(
                        ...
                        유령이_콜드_램프에_겹쳐_실린다(겹친_회차[0], 겹친_회차[1]),
                        겹침이_창_안에서_끝난다(겹친_회차[2])))
    ...
    /** 유령의 몫이 콜드 램프값과 같은 회차에 실린다. 그 차이가 곧 없는 인스턴스의 몫이다. */
    private Optional<String> 유령이_콜드_램프에_겹쳐_실린다(long 겹친_회차, long 유령_없는_회차) {
        if (유령_없는_회차 != 바닥) {
            return Optional.of(("전제 — 갓 뜬 뒷단만 있는 회차가 %d 다. 바닥 %d 이 아니면 "
                    + "콜드 램프가 안 걸린 것이라 겹침이 안 만들어진다")
                    .formatted(유령_없는_회차, 바닥));
        }
        return 겹친_회차 == 여유 ? Optional.empty()
                : Optional.of("겹친 회차가 %d 다 — 죽은 유령의 몫 %d 가 그대로 실려야 한다"
                        .formatted(겹친_회차, 여유));
    }
     ...
     *
     * <p>범위가 아니라 값으로 본다. "줄었다" 만 보면 하한까지 떨어진 회차도 통과한다.
     */
    private Optional<String> 겹침이_창_안에서_끝난다(long 창_밖) {
        return 창_밖 == 램프_4초 ? Optional.empty()
                : Optional.of("창을 넘긴 회차가 %d 다 — 갓 뜬 뒷단의 램프 몫 %d 여야 한다"
                        .formatted(창_밖, 램프_4초));
    }
}
