...
public final class SnapshotHolder {
     ...
     * 경합이 생긴다. 참조 교체는 원자적이고 스냅샷은 불변이라 이걸로 족하다.
     */
    private volatile GatewaySnapshot current = GatewaySnapshot.EMPTY;
    ...
    public Duration dataAge() {
        return Duration.between(current.publishedAt(), clock.instant());
    }
