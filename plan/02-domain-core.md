...
| 위 + Redis도 죽어 큐 등록 실패 | **거절** — 줄이 있거나 모른다. 줄이 비었다고 아는 한산 갈래만 상한 안 fail-open (CY-1006) |
...
**등록이 실패해도 줄이 있거나 모르면 열지 않는다 (CY-1006).** 줄에 세울 수 없다는 것도 모름의 한 종류다.
열면 새로 온 사람이 레디스에 순번을 쥔 사람을 앞지른다. 상한 안 fail-open 은 줄이 비었다고 아는 한산
갈래(9번 — 신선한 재료에 IDLE)만 탄다. 대가는 레디스 장애 중 줄이 선 쿠폰의 신규 통과가 0 이라는 것이다.
...
12. **완료 (CY-1006)** **등록 실패의 fail-open 은 한산 갈래만** — 그 밖의 줄 판정은 `REJECT_OVERLOAD`
