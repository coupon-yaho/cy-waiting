...
| 계층 | 범위 | 도구 | 기준 |
|---|---|---|---|
| **단위** | 순수 도메인 | JUnit 5 | 브랜치 100%, 뮤테이션 생존 ≤ 10% (아래) |
| **통합** | 실제 Redis·Lua | Testcontainers | 브랜치 80% |
| **컨텍스트** | 라우트·필터·배선 | `@SpringBootTest` | 판정 분기 100% |
| **카오스** | 장애·회복 | Toxiproxy | 시나리오 전수 |
| **부하** | 처리량·지연 | k6 | 게이트 기준 |

> **생존 10% 는 PIT 의 임계로는 안 지켜진다.** PIT 은 정수 백분율로 비교해
> 89.9% 를 90 으로 올려 통과시킨다. 반올림 없는 비율은 뮤테이션 잡이 리포트를
> 직접 세어 다시 문다 — 그 잡이 이 표의 값을 지키는 자리다.
...
## TS-12 · MUST · 판정이 무는지 뮤턴트로 확인한다

판정을 쓴 직후 **구현에 뮤턴트를 직접 넣어 빨개지는지 본다.** 안 죽으면 그
사실을 저널과 계획서에 적는다 — 판정을 지우지 말고, 재는 척하는 자리임을 남긴다.

카오스 시나리오에서 특히 크다. 장애를 재는 시험이라 초록이 그럴듯해 보이고,
아무것도 안 재는 판정이 오래 살아남는다. Phase 8 구간의 리뷰가 매번 같은
여덟 가지를 찾았다.

1. 관측만 하고 판정에 안 걸린 값 — 대입만 되고 어느 위반 목록에도 안 들어간다
2. 판정이 다른 대상을 본다 — 회복 판정이 대조군 목록을 봤다
3. 비교 집합에서 실패 사례를 뺀다 — 사라진 사람을 제외해 정의상 안 깨진다
4. `0 == 0` — "끝난 뒤 0" 만 봐서 막으려던 장치를 들어내도 통과한다
5. 범위만 보고 값이 갈리는지는 안 본다 — 밴드 안 상수가 지터로 통과한다
6. 기다린 것과 판정이 그것을 본 것은 다르다 — 낡음을 기다렸지만 그 갈래를
   밟았는지는 안 쟀다
7. 픽스처가 프로덕션에 없는 상태를 만든다 — TS-3 이 막으려던 것이다
8. 시험이 프로덕션이 안 부르는 경로를 부른다 — 고쳤다고 적은 것이 한 번도 안 돌았다
9. 막는 쪽만 보고 받는 쪽을 안 본다 — 범위 밖을 거절하는 시험만 있으면 경계가
   한 칸 안으로 밀려도 초록이라, 정상값이 조용히 거절되기 시작한다

8번은 구현 위치를 재고 있다는 뜻이다. 배선을 실제로 조립해 부르는 자리로 옮기면
그때 빨개진다.

**뮤턴트를 넣었으면 그 시험만이 아니라 스위트 전체를 돌린다.** `chaosTest` 만
돌리면 "이 판정이 문다" 는 알아도 "이 판정이 없으면 안 물린다" 는 모른다. 둘이
같이 죽으면 그 시나리오의 고유 기여는 0 이고, 그것은 결함이 아니라 사실이다 —
저널에 그렇게 적는다. 여기서 갈라지지 않아 "구멍을 닫았다" 고 잘못 적은 저널이
이미 둘 있다 (AIJ-0183).

**결함을 통과 조건으로 삼지 않는다.** 지금 값이 잘못돼 있다는 것을 알면서 그 값을
등호로 단언하면, 고치는 순간 시험이 빨개져 **수정을 막는 시험**이 된다. 상한·하한
부등호로 적어 고친 뒤에도 통과하게 두고, 지금 값은 계획서와 게이트에 남긴다.
