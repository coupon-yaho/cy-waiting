...
**현재 상태**: 제로베이스 재작성 중. **Phase 10(Scale-out) 부분 충족 마감 — v0.8.0 (2026-09-22), 라우팅 선결 항목을 채운 v0.9.0 (2026-09-23).** 샤딩은 근거가 없어 안 열었고(O-12), 틱 지연과 증설 효율은 우리 쪽 계측 둘(CY-990)과 두 번째 기계가 남았다.
Phase 1~7 은 닫혔고 Phase 8·9 는 부분 충족으로 닫았다 (90-decisions G-1). Phase 9 는 검증(9.4)을 다 쟀고 그 과정에서 **제품 결함 아홉**이 나왔다 — 전부 시험이 초록인 채로 있던 것들이다. 여덟째·아홉째는 게이트웨이 둘로 회복 구간에 실부하를 넣어 나왔다: RC3 미달과, 승계 뒤 첫 틱이 램프를 안 타던 것이다. 파 보니 **RC3 와 RC4 가 같은 원인으로 충돌한다** — 프로브가 줄을 통해서만 나가 폴링 간격 하나가 두 게이트를 반대로 민다. 고칠 자리는 합성 프로브다 (AIJ-0249). RC4 상한은 관측으로 걸어 봤지만 진동해서 뺐다 (AIJ-0250). 게이트웨이 둘에서 쏠림까지 재고 기본 전략을 라운드로빈으로 뒤집었다. 미달 둘은 성격이 다르다: 하나는 P2C 를 기본에서 뺀 근거이고, 하나는 뒷단이 보고에 `inFlight` 를 실어야 닫힌다. **라우팅은 기본이 꺼짐이고, 켜기 전에 읽을 표가 계획서에 있다.** 무엇이 남았는지는 각 게이트 표가 든다. 브랜치는 태스크 단위로 딴다 (WF-3).
