...
**현재 상태**: 제로베이스 재작성 중. **Phase 9(Routing) — v0.6.0. 검증 여섯을 다 쟀고 뒷단 계약을 기다리는 게이트 하나만 남았다.**
Phase 1~7 은 닫혔다. Phase 8 은 부분 충족으로 닫았고, Phase 9 는 검증(9.4) 여섯을 다 쟀다 — **라운드로빈으로는** 전부 넘겼고(종료 시 유출과 앓는 대 배제까지), P2C 는 실측을 막는 창을 찾았으나 아직 그 안에서 재지 못했다. 콜드 스타트 초기값은 뒷단 계약이 선행이다. 무엇이 남았는지는 각 게이트 표가 든다. 브랜치는 태스크 단위로 딴다 (WF-3).
