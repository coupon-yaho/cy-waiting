...
# 죽은 주소. **갈래마다 예외가 다르고 재시도 조건이 갈린다.** netty 는 연결
# 단계 실패를 셋으로 나누고(거절 · 도달 불가 · 그 밖의 소켓 오류), 이름 풀이
# 실패는 그 앞 단계라 또 다르다.
#   - 이름은 풀리고 포트만 닫힘: 즉시 거절(ECONNREFUSED)
#   - 라우팅이 안 되는 주소: **커널이 즉시 거절한다** — 실측 0.52 초, 도달
#     불가(EHOSTUNREACH)다. 시간 초과를 기대했는데 아니었다. 다만 그 대에
#     경로 자체가 없으면 ENETUNREACH 로 갈리고, 그건 지금 안 덮여 있다
#   - 안 풀리는 이름: 이름 풀이 실패라 연결 이전이다
# 기본은 즉시 거절이고, 도달 불가는 `DEAD_ADDR=10.255.255.1:8090` 으로 부른다.
#
# **어느 갈래를 쟀는지는 폴백 원인 로그가 말한다.** `GATEWAY_LOG_LEVEL=DEBUG`
# 로 부르면 예외 이름이 찍힌다 — 주소만으로는 이 머신의 라우팅 테이블에
# 달렸으므로, 다른 대에서 같은 스크립트가 다른 갈래를 잰다.
