...
000"
# **두 하한을 문구로 갈라 잡는다.** 둘 다 "판정 불가" 를 내므로 종료 코드나
# 그 말만 보면 어느 가드가 물었는지 모른다 — 실제로 표본 하한을 재려던 사례가
# 도착 하한을 재고 있었다.
bare "기본 최소 표본은 20 이다" 2 "표본 19 건으로는" "$(many 200 19)"
bare "기본 최소 도착은 20 이다" 2 "죽은 대로 가는 경로" "$(many 200 15)
$(many 202 100)"
