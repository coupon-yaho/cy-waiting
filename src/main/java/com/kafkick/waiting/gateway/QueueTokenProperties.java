 ...
 * @param previous 검증에서만 받아 주는 옛 키. <b>롤링 배포의 창이다</b> — 없으면
 *                 새 키 파드가 낸 토큰을 옛 키 파드가 거절해 그 사람이 자리를
 *                 잃는다 (CY-902). 배포가 끝나면 뺀다
 */
...
public record QueueTokenProperties(String secret, List<String> previous) {
    ...
    public QueueToken queueToken() {
        return QueueToken.of(secret, previous);
    }

    public EntryToken entryToken() {
        return EntryToken.of(secret, previous);
    }
    ...
    public String toString() {
        return "QueueTokenProperties[secret=***, previous=%d개]".formatted(previous.size());
    }
}
