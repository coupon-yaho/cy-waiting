...
public final class FilterOrder {
    ...
    /**
     * 라우트 안의 판정. <b>서킷 앞이다</b> — 뒤로 가면 서킷이 열린 동안 판정이 안 돌아
     * 래치가 표식을 못 받고, 다음 창의 신규 유입이 방금 줄 선 사람을 추월한다. 라우트
     * 필터는 전역 필터와 다른 사슬이라 위 값들과 겹쳐도 된다.
     */
    public static final int ROUTE_ADMISSION = 0;

    /**
     * 라우트 안의 서킷. <b>응답을 쓰는 필터보다 안쪽이어야 한다.</b> 프레임워크의 쓰기
     * 필터가 {@code -1} 이라, 그보다 앞이면 폴백이 이미 커밋된 응답에 쓰려 들고 읽기
     * 전용 헤더에서 터진다 — 그 예외가 원래 실패를 덮는다.
     */
