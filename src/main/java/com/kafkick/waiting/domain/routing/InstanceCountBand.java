...
/**
 * 인스턴스 수가 가정한 10~20 대 안인가. <b>자동으로 전략을 안 바꾼다</b> — 인스턴스
 * 수로 전환하면 임계 근처에서 진동하고(롤링 배포가 그 구간을 지난다), 어느 구간이
 * 무슨 모드였는지가 대시보드에 안 남아 장애 분석이 막힌다. 알리고 판단은 사람이 한다.
 */
public enum InstanceCountBand {
    ...
    /** 가정이 정한 하한. 이보다 적으면 무작위 둘을 뽑는 이득이 없다. */
    private static final int LOW = 10;

    /** 가정이 정한 상한. 이보다 많으면 후보를 다 훑는 비용이 다시 문제가 된다. */
