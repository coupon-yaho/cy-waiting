...
public record Tunables(double idleCreditRatio, long inFlightSeconds) {
    ...
    /**
     * 한산 몫의 하한. <b>0 은 못 받습니다</b> — 피크 전량이 큐 등록으로 가고, 그것이
     * 요청 경로에서 레디스를 치는 유일한 예외 경로입니다. 되돌리려면 그 레디스에
     * 써야 하니 값 하나로 스스로 못 빠져나오는 상태가 됩니다.
     */
    ...
    /**
     * 걸림 시간의 하한. <b>서킷의 느림 임계보다 커야 합니다</b> — 작으면 느려진 뒷단이
     * 서킷에 집계되기 전에 격벽이 먼저 끊어, 서킷이 영영 안 열리고 회복 경로가 사라집니다.
     */
    public static final long MIN_INFLIGHT_SECONDS = 2;

    /**
     * 걸림 시간의 상한. <b>자리를 놓게 하는 시한과 같은 값입니다</b> — 그보다 길면 자리가
     * 시한에서 강제로 반납되므로, 존재할 수 없는 동시 건수를 상한으로 삼는 셈입니다.
     */
    ...
    /**
     * 운영자가 적은 값을 읽습니다. <b>깨져도 멈추지 않습니다</b> — 장애 중에 손으로
     * 넣는 값이라 오타가 나는데, 그때 기동이 막히면 되돌릴 수단이 사라집니다. 한 값이
     * 틀렸다고 나머지를 버리지도 않습니다: 오타 하나가 방금 고친 값도 되돌립니다.
     */
    ...
    /**
     * 스냅샷에 실어 보낼 모양. <b>읽는 쪽과 같은 형식이어야 합니다</b> — 갈리면 리더가
     * 실은 값과 노드가 읽는 값이 달라지고, 그 차이는 값을 바꿔 본 뒤에야 드러납니다.
     */
