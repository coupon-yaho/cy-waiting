...
--
-- **지우는 것을 줄과 생존 신호로 좁힌다.** 임계를 지우면 입장한 사람이 토큰을 두 번
-- 받고, 바닥값을 지우면 승격된 복제본에서 줄 선 사람이 통째로 추월당하며, 유예에는
-- 차례가 왔던 사람이 종료를 안 받게 막는 입장 표시가 있다.

-- **여기서 슬롯을 검사하지 않는다.** 클러스터는 KEYS 의 슬롯을 본문보다 먼저 계산해
-- 갈리면 CROSSSLOT 으로 거절하고, 단독 배치는 슬롯 개념이 없어 갈려도 문제가 없다.
...
-- 울타리 표가 아직 없는 쿠폰이 그대로 지워진다.
if fence <= 0 then
    return 0
end
-- **옛 리더의 명령은 안 듣는다.** 검사와 삭제 사이에 리스가 끝나면 옛 리더가 새 리더의
-- 줄을 지우므로, 줄과 같은 슬롯에 펜스 번호를 남기고 그보다 옛 임기를 거절한다.
...
-- 없고, 그게 울타리가 지키려던 바로 그 경우다. 수명을 주는 것은 쿠폰이 활성에서 빠진
-- 뒤에도 표가 남으면 시계가 뒤로 간 리더가 자기 시각이 옛 표를 넘어야 풀리기 때문이다.
...
-- **못 읽으면 안 지운다.** 못 읽은 것은 매진이 아니다. 여기서 지우면
-- 재고 키를 잃은 쿠폰의 줄이 통째로 사라지고 되살릴 방법이 없다.
if raw == false then
    return 0
end
-- **담는 쪽과 정확히 같은 모양만 읽는다.** 관대하게 읽으면 수집이 미상이라 부른 값을
-- 삭제가 매진으로 보고 그 줄을 영영 지우고, 좁게 읽으면 매진인 쿠폰이 영영 안 지워져
-- 죽은 줄이 폴링 예산을 먹는다 — 갈리는 자리가 하필 되돌릴 수 없는 쓰기다.
