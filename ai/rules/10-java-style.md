...
| 빌드 스크립트 선언 이름 | **영문** | 태스크 이름은 사람이 `./gradlew` 로 친다 |

**테스트도 타입 이름은 영문이다.** `TS-2` 가 허용하는 것은 시험 *이름* 과 헬퍼이지
클래스·레코드명이 아니다. 파일 단위로 예외를 주면 최상위든 `@Nested` 든 다 샌다 —
실제로 `class 필수값`·`class IDLE에줄이없다` 같은 것이 그렇게 들어갔다.

**빌드 스크립트는 훅이 아니라 PR·CI 가 본다.** `check-java.sh` 는 `.java` 만 받는다.
`.github/scripts/build-script-identifiers.sh` 가 `def`·`task`·타입 있는 선언·
`ext` 대입·`tasks.register('이름')` 을 보고, 러너와 CI 가 같이 부른다. 클로저
파라미터와 동적 이름은 못 본다 — 그 한계는 스크립트 머리에 적혀 있다.
...
까지 갔다. **`MUST` 로 올리고 훅에 넣었다** — 주석·문자열·문자 리터럴을 지운 뒤
ASCII 밖 글자가 남으면 위반이다.

한 번에 다 막히지 않았다. 시험 파일을 통째로 제외한 탓에 클래스명이 샜고,
`.java` 만 보는 탓에 `build.gradle` 이 통째로 빠져 있었다. **범위를 파일이 아니라
선언 종류로 그어야** 구멍이 안 남는다.
