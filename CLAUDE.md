...
Phase 1~7 은 닫혔고 Phase 8·9 는 부분 충족으로 닫았다 (90-decisions G-1). Phase 9 는 검증(9.4)을 다 쟀고 그 과정에서 **제품 결함 여덟**이 나왔다 — 전부 시험이 초록인 채로 있던 것들이다. 여덟째는 게이트웨이 둘로 회복 구간에 실부하를 처음 넣어 나왔다: **회복이 48초로 RC3 의 30초를 넘고, 늦는 것은 서킷이 표본을 모으는 속도다** (AIJ-0246). 게이트웨이 둘에서 쏠림까지 재고 기본 전략을 라운드로빈으로 뒤집었다. 미달 둘은 성격이 다르다: 하나는 P2C 를 기본에서 뺀 근거이고, 하나는 뒷단이 보고에 `inFlight` 를 실어야 닫힌다. **라우팅은 기본이 꺼짐이고, 켜기 전에 읽을 표가 계획서에 있다.** 무엇이 남았는지는 각 게이트 표가 든다. 브랜치는 태스크 단위로 딴다 (WF-3).
