...
class ClusterEjectionTest {
    ...
    void 과반이_뺀_인스턴스만_고른다() {
        assertThat(ClusterEjection.of(5, Map.of("x", 3, "y", 2))).containsExactly("x");
    }

    @Test
    @DisplayName("절반은_과반이_아니다")
    void 절반은_과반이_아니다() {
        assertThat(ClusterEjection.of(4, Map.of("x", 2))).isEmpty();
    }

    @Test
    @DisplayName("분모는_실은_수가_아니라_산_수다")
    void 분모는_실은_수가_아니라_산_수다() {
        assertThat(ClusterEjection.of(30, Map.of("x", 2))).isEmpty();
    }

    @Test
    @DisplayName("한_대뿐이면_그_한_대가_전부다")
    void 한_대뿐이면_그_한_대가_전부다() {
        assertThat(ClusterEjection.of(1, Map.of("x", 1))).containsExactly("x");
    }
    ...
    void 관측이_없으면_아무것도_안_뺀다() {
        assertThat(ClusterEjection.of(0, Map.of("x", 1))).isEmpty();
    }
}
