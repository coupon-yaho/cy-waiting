...
k6 run --quiet --summary-export=/tmp/r1-contended.json \
    test/load/contended-coupon.js >/tmp/r1-contended.log 2>&1 &
hot=$!

# 램프가 올라간 뒤에 재기 시작한다. 겹치기 전에 재면 격리를 안 잰 것이다.
sleep 4
k6 run --quiet --summary-export=/tmp/r1-idle.json \
    test/load/idle-coupon.js >/tmp/r1-idle.log 2>&1
idle_run=$?
wait "$hot"
...
echo
echo "-- 경합 쿠폰 --"
test/load/evaluate-gate.sh contended-coupon /tmp/r1-contended.json || failed=1
echo
echo "-- 한산 쿠폰 (경합 중) --"
test/load/evaluate-gate.sh idle-coupon /tmp/r1-idle.json || failed=1

# k6 자체가 못 돈 것을 통과로 세지 않는다.
if [[ $idle_run -ne 0 ]]; then
    echo "::error title=R1 실측::한산 시나리오가 임계를 넘겼다 — /tmp/r1-idle.log"
    failed=1
fi
