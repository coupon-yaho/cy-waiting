...
class SnapshotMetricsTest {
    ...
    /**
     * <b>루프가 도는 것과 재료를 받은 것은 다르다.</b> 받아오기에 실패해도 루프는
     * 돌므로(공유 장애를 노드별 신호로 안 흘리려고) 첫 틱은 곧 지나간다. 기준선은
     * 여전히 EPOCH 라, 첫 틱만 보고 가르면 그때부터 17억이 나간다. 헬스는 같은
     * 상황을 -1 로 내므로 두 창구가 서로 다른 말을 하게 된다.
     */
    @Test
    @DisplayName("루프만_돌고_못_받았으면_나이가_음수다")
    void 루프만_돌고_못_받았으면_나이가_음수다() {
        지표를_건다();
    ...
    /** <b>안 실려 오면 음수입니다.</b> 그때는 각 노드가 자기 기동값으로 돕니다. */
    @Test
