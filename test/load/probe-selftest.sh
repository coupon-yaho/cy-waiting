...
# **한쪽만 빠진 표본을 버릴 때 나머지도 비우는지 본다.** 안 비우면 남은 값이
# 다음 표본의 새 값과 짝지어져 창이나 CPU 차분이 두 틱어치가 된다 — 값이
# 조용히 절반이나 두 배가 되는데, 표본 수는 그대로라 눈에 안 띈다.
#
# 시각이 빠진 회차 뒤의 표본이 두 틱이 아니라 한 틱으로 잡혀야 한다.
run_probe '1788587000 0
x
1788587000 400000' '0
50
...
0.05
0.1'
expect "시각이 빠진 뒤에도 한 틱으로 잰다" "250" 3

# **버린 표본의 남은 값이 다음 표본과 짝지어지면 안 된다.** 위 둘은 남은 값이
# 다음 틱에 덮여서 비우든 안 비우든 같다. 갈리는 것은 **한쪽이 남은 뒤 반대쪽이
# 빠지는** 조합이다 — 그때 두 틱 전의 CPU 가 이번 시각과 짝지어진다.
run_probe '1788587000 0
x
1788587000 400000' '0
50
