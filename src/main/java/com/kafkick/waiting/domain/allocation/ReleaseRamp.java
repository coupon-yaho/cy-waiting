 ...
 * 서킷이 닫히는 한 틱에 값이 원래 몫으로 그대로 돌아간다 (AIJ-0245). <b>회복 구간에만
 * 걸지 않는다</b> — 그러면 그 구간이 끝나는 순간이 새 계단이 된다 (AIJ-0249).
 */
public final class ReleaseRamp {

    /**
     * 한 회차에 몫이 오를 수 있는 배수. <b>제약은 RC4 가 아니라 RC3 이다</b> — 드는
     * 틱이 {@code log(상한/하한)} 이라 상한이 크면 그만큼 는다. 근거로 잰 두 회차는
     * 조건이 서로 달라, 기본 조건에서 되풀이해 확인한 값이 아니다 (AIJ-0248).
     */
     ...
     * 뒤의 출발점으로 쓴다. 정책 하한 아래로 깎으면 한산 통과 상한이 0 이 된다 (R1).
     *
     ...
     * {@code floor} 음수에 램프를 켜 넣으면 다음 회차가 무제한이 된다 (DS-2).
     */
