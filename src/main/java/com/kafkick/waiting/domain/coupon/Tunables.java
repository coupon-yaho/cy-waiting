...
public record Tunables(double idleCreditRatio, long inFlightSeconds) {
    ...
    /** 한산 몫의 하한. <b>0 은 못 받습니다</b> — 피크 전량이 큐 등록으로 갑니다. */
    // 그게 요청 경로에서 레디스를 치는 유일한 예외 경로이고, 되돌리려면 그
    // 레디스에 써야 한다 — 값 하나로 스스로 못 빠져나오는 상태가 된다.
    public static final double MIN_IDLE_RATIO = 0.1;

    /** 한산 몫의 상한. 한산이 거의 다 긁으면 차례가 온 사람이 밀립니다. */
    // 둘이 같은 노드 예산을 쓴다. 토큰 수명이 지나면 줄 맨 뒤로 다시 서고,
    // 그건 순번 역행이다 (불변식 3).
    public static final double MAX_IDLE_RATIO = 0.9;

    /** 걸림 시간의 하한. <b>서킷의 느림 임계보다 커야 합니다.</b> */
    // 작으면 느려진 뒷단이 서킷에 집계되기 전에 격벽이 먼저 끊는다 — 서킷이
    // 영영 안 열려 회복 경로 자체가 사라진다.
    public static final long MIN_INFLIGHT_SECONDS = 2;

    /** 걸림 시간의 상한. <b>자리를 놓게 하는 시한과 같은 값입니다.</b> */
    // 그보다 길면 존재할 수 없는 동시 건수를 상한으로 삼는 셈이다 — 자리는
    // 시한에서 강제로 반납된다.
