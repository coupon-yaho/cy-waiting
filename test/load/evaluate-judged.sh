...
printf '  %-24s %s%%\n' "판정한 요청 비율" "$pct"
printf '  %-24s %s%%\n' "기준" "$target"
# **비율이 이 회차에서 몇 건인지 같이 낸다.** 30일 창에서 정한 수라(O-7) 짧은
# 회차에 그대로 쓰면 뜻이 달라진다 — 낡음 창은 한 번에 수만 건이라 그 창에서
# 이 기준은 사실상 "낡음 창 0회" 다. 백분율만 찍으면 그 사실이 뒤에 숨는다.
#
# **판정과 같은 값이 나와야 한다.** `100 - 99.9` 가 0.09999999999999432 라 그냥
# 자르면 20 이 19 로 나온다 — 그 수를 보고 스무 건까지 된다고 읽으면 안 된다.
budget=$(awk -v n="$total" -v t="$target" \
    'BEGIN{ printf "%d", int(n * (100 - t) / 100 + 1e-6) }')
printf '  %-24s %s건\n' "이 회차의 열화 예산" "$budget"
