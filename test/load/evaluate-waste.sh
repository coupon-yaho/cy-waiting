...
# 표본 밖인 판이 통과한다 — 스위퍼가 한 명도 안 걷어도.
UNKNOWN_MAX="${UNKNOWN_MAX:-0.05}"

# **스위퍼에게 기회가 있었던 인원의 하한.**
#
# "못 피하는 유령이 있는가" 를 묻는 것은 방향이 반대다 — 판이 퇴화해서 아무도
...
[ "$admitted_total" -ge "$MIN_ADMITTED" ] \
    || fail "차례가 $admitted_total 명뿐이다 — 판이 짧거나 크레딧이 안 나갔다"
# **못 피하는 유령이 있어야 기구를 잰 판이다.** 하나도 없으면 이탈이 안
# 일어났거나 판이 수명보다 짧아, 기구가 아무 일도 안 하고 통과한다.
sweepable=$((avoidable + swept))
[ "$sweepable" -ge "$MIN_SWEEPABLE" ] \
    || fail "스위퍼에게 기회가 있었던 인원이 $sweepable 명뿐이다 — 이 판은 기구를 안 잰다"
awk -v u="$unknown" -v t="$total" -v m="$UNKNOWN_MAX" \
    'BEGIN { exit (t == 0 || u / t <= m) ? 0 : 1 }' \
    || fail "$total 명 중 $unknown 명을 못 되짚었다 — 표본이 판을 못 덮는다"
