...
/**
 * 판정이 지금 무엇을 보고 있는지를 지표로 냅니다. 안 내면 사고가 났을 때 어떤 재료로
 * 판정했는지를 사후에 재구성할 수 없습니다 — 요청당 로그를 안 남기기 때문입니다 (LG-1).
 */
public final class SnapshotMetrics {
    ...
    /**
     * <b>강한 참조로 등록합니다.</b> {@code MeterRegistry.gauge(name, obj, fn)} 는 대상을
     * 약한 참조로 잡으므로 여기서 만든 객체가 첫 GC 에 수거되고 그 뒤로는 지표가 영원히
     * {@code NaN} 을 냅니다 — 스크레이프에 줄은 그대로 나가 이름만 보는 시험은 못 잡습니다.
     */
    ...
    /**
     * 적용 중인 한산 몫. <b>{@code -1} 은 "안 실려 왔다" 는 뜻입니다.</b> 운영 값으로 0 을
     * 못 넣으므로 이 게이지가 0 을 낼 일은 없습니다 — 음수만이 미전파를 뜻합니다.
     */
    ...
    /**
     * 판정이 낡음으로 넘어가는 순간을 짚는 값입니다. <b>못 받았으면 -1 입니다</b> — 기준선이
     * {@code Instant.EPOCH} 라 그대로 내면 17억이 나갑니다. 루프가 돌았는지가 아니라
     * <b>재료를 받았는지</b>로 가릅니다. 실패해도 루프는 돌기 때문입니다.
     */
    ...
    /**
     * <b>널 검사를 안 겹칩니다.</b> 틱 시각이 없는 것은 초기값 하나뿐이고 그것은
