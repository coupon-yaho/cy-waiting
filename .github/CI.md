...
못 묻는 것과 아예 없는 저장소를 못 묻는 것이 구별되지 않는다 — 앞은 공급망
신호가 아니고 뒤는 그 자체가 사고인데 응답이 같다. 막으면 게이트가 흔들리고,
넘기면 공격자가 없는 저장소를 적어 지나간다.

원격에 묻는 일은 갱신으로 옮겼다. 핀을 바꾸면 같이 돌린다.
...
가 확인합니다. 없는 이름을 보는 패널은 영원히 빈 화면인데, 대시보드를 열어 보기
전까지 아무도 모릅니다. 지표 이름을 바꿀 때 특히 그렇습니다 (LG-11).

## Loki 라벨

로그 라벨은 허용 목록 안에서만 씁니다 — `app`·`env`·`instance`·`level`·`logger`·
`coupon_version`. Loki 는 라벨마다 스트림을 만들기 때문에, 쿠폰 식별자처럼 가짓수에
상한이 없는 값을 넣으면 인덱스가 무너집니다 (LG-4).

`.github/scripts/loki-labels.sh` 가 `docker/observability/promtail.yml` 을 읽어
확인합니다. 목록을 늘리려면 **그 값의 가짓수에 상한이 있는지 먼저 따져야** 하고,
그 판단을 이 검사가 강제합니다.

**사람이 리뷰로 막을 수 없습니다.** 라벨 하나 추가는 한 줄이고, 그 한 줄이 무엇을
뜻하는지는 운영에서 인덱스가 죽고 나서야 드러납니다.

> **워크플로는 기본 브랜치에 있어야 돕니다.** `issue_comment` 로 도는 워크플로는
> GitHub 이 항상 기본 브랜치의 것을 씁니다. 기능 브랜치에만 두면 코멘트를 달아도
> 아무 일도 안 일어나고, 그 사실이 어디에도 안 드러납니다.
...
40분을 기다리게 하면 아무도 PR 을 안 연다. **대신 nightly 가 develop 도 잰다.** 스케줄은
기본 브랜치(main)에서만 돌아, 그것만 두면 develop 에 쌓인 회귀를 릴리스 PR 에서야 본다.
빨개지면 잡 요약의 "뮤턴트 위치" 가 살아남은 것과 판정 안 선 것을 클래스·메서드·줄로
낸다. 리포트는 7일 보관한다.
