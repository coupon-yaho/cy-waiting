...
--
-- **메모리 상한에서도 돈다 (첫 줄).** 연장은 메모리를 안 늘려 거부되지 않는데 신규 획득의
-- SET·INCR 은 거부된다. 그러면 리더가 내려간 순간 아무도 못 잡아 배분이 영영 멎는다 —
-- 메모리가 줄어야 풀리는데 줄이는 청소를 리더가 돈다. 쓰는 것은 키 둘뿐이라 넘치는 양이 없다.
--
...
--
-- **그 근거는 씨앗을 한 번만 주던 설계의 것이다.** 지금은 획득할 때마다 바닥을 다시
-- 깔아 나중에 잡은 쪽이 항상 크므로, 앞 방향은 창 없이도 선다. 그래도 둔다 — 두 버전이
-- 같은 창을 더하는 동안 순서에 영향이 없고, 빼면 세는 값을 잃은 뒤 섞인 함대에서 옛
-- 노드가 더 큰 번호를 매겨 새 노드가 표 수명 내내 막힌다. 빼서 얻는 것이 없다.
--
