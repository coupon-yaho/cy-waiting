...
**현재 상태**: 제로베이스 재작성 중. **Phase 9(Routing) — v0.6.0. 구현은 다 들어갔고 검증이 남았다.**
Phase 1~7 은 닫혔다. Phase 8 은 부분 충족으로 닫았고, Phase 9 는 구현이 끝나고 검증(9.4)이 남았다 — 무엇이 남았는지는 각 게이트 표가 든다. 브랜치는 태스크 단위로 딴다 (WF-3).
