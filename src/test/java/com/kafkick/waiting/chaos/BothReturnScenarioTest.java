...
class BothReturnScenarioTest {
    ...
    private long 노드당(long 크레딧) {
        ...
        // **발행이 하는 순서를 그대로 밟는다** — 반올림, 그다음 하한.
        // 잘라 버리면 프로덕션과 1 씩 갈리고, 하한을 빼면 크레딧이 하한 아래로
        // 내려앉는 구간에서 이 시나리오만 다른 값을 정상값으로 읽는다.
        long 반올림 = Math.round(가라앉은);
        long 발행값 = Math.max(반올림, Math.max(0, 수집기.lastFloor()));
        // 발행의 셋째 단계인 서킷 조임은 여기서 항등이다 — 이 시나리오는 서킷을
        // 안 세운다. 세우면 재는 것이 노드당 몫이 아니라 서킷이 된다.
        return 발행값 / 분모.count();
    }
