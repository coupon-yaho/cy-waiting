...
public class AdmissionDecider {
     ...
     * 경로가 따로 카운터를 들면 한 초에 두 예산이 겹쳐 나간다 (F4). 쿠폰 키에
    ...
    /**
     * 사다리 6번이 보는 참. 폴백을 안 탄다 (AIJ-0073). 줄이 있을 때만 재는 것은
     * 한산한 쿠폰의 credit 이 0 이라 {@code 0 >= 0} 이 참이 되기 때문이다 — 그러면
     * R1 경로가 통째로 막힌다.
     */
     ...
     * 없다. 배분 전 credit 0 구간이면 대기자 한 명에 전원이 거절된다 (C-8).
     */
    ...
    public AdmissionDecision decide(AdmissionRequest req) {
        ...
        //     신규 유입이 그 줄을 추월하고 재고까지 먼저 먹는다 (불변식 4).
        ...
        // 6' — 서킷이 열렸다 (F3). 통과를 내도 서킷이 가로채 503 이 되니 줄로 보내야
        //      회복 뒤 크레딧으로 빠진다. **줄 가드 뒤·낡음 앞이다** — 올리면 줄에 선
        //      적 없는 사람이 추월하고(불변식 4), 내리면 모름이 앎을 이긴다.
    ...
    public long admittedRatePerSec(AdmissionDecision decision, CouponState state,
        ...
        // IDLE 쿠폰이고 IDLE 이면 credit 이 0 이다 (I1). 그 값으로 재면 한산한
    ...
    /**
     * 한산 통과에 쓸 몫. <b>재료에 실려 온 값을 먼저 봅니다</b> (P-1). 배포 없이
     * 되돌릴 수 있어야 롤백이 성립하고, 그 전파 경로가 스냅샷입니다.
     */
