...
### 개별 태스크(`2.3.9`)는 Jira 이슈로 만들지 않는다
...
**계획서 ID 가 이미 추적을 하고 있다.** Jira 에 60여 개를 복제하면 계획이 바뀔 때마다
둘이 갈라지고, 백로그도 읽을 수 없게 된다. 세부 항목은 Task 설명에 체크리스트로 넣어
보이게만 한다.
...
페이즈 문서의 각 태스크 절이 Task 하나가 된다. **세부 태스크(`2.3.9`)는 이슈로
만들지 않고** 그 Task 설명의 체크리스트로 들어간다 — 하위 작업까지 복제하면
계획이 바뀔 때마다 둘이 갈라진다.
