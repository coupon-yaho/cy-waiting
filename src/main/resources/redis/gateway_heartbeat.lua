...
--            field = '#r:'..id       → 그 노드가 조회를 못 받아 주는 중이면(상한 거절·조회 실패) ",1,"
-- ARGV[1]  instanceId
...
-- ARGV[7]  지난 하트비트 뒤로 조회를 못 받아 줬으면 "1". 아니면 비우고 그 field 를 지운다
--
...
-- 하나뿐이다. 쓰는 양은 노드 수에만 비례하고, 쓰기와 정리가 한 스크립트라 넘치는 양이 없다 (RD-12) —
-- 둘을 떼면 이 안전성도 같이 깨진다.
--
-- **서킷을 별도 field 에 싣는다.** 값에 붙이면 옛 노드의 tonumber 가 nil 을
-- 내고 그 노드를 죽은 것으로 보고 지운다 — 분모가 줄어 남은 노드가 큰 몫을 쓰는 초과
-- 발급 방향이다. 나누면 옛 노드는 표만 지우고 다음 틱에 다시 실린다 (없는 표는 CLOSED).
--
...
local VOTE = '#c:'
-- **회복 봉우리를 정상과 견줄 재료다.** 봉우리가 정상의 1.2배를 넘으면 안 되는데,
...
-- 남고, 그러면 남은 노드가 큰 몫을 쓴다. **아는 것만 받는다** — 여기만 그대로 이어
-- 붙이면 모르는 값이 표로 세어져 전 클러스터의 배분을 멈출 수 있다.
local mine = ARGV[3]
...
-- **순서를 배열로 고정한다.** pairs 는 순서가 정해져 있지 않아 HDEL 인자 순서가 매번
-- 달라진다 — 효과는 같아도 재현이 안 되는 것을 스크립트에 남길 이유가 없다.
local ids = {}
local votes = {}
local passes = {}
local seenOf = {}
...
local reported = 0
local passSum = 0
-- **합만으로는 "진짜 0" 과 "아무도 안 실었다" 가 안 갈린다.** 롤아웃 중 옛 노드가 새
