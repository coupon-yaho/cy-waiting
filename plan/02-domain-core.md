...
| 위 + Redis도 죽어 큐 등록 실패 | **거절** — 줄에 못 세우는 것도 모름이다. 어떤 줄 판정이든 열지 않는다 (CY-1006) |
...
순번은 받고, 줄이 차면 429 다. 토큰을 쥔 사람은 계속 통과한다.

**등록이 실패하면 열지 않는다 (CY-1006).** 줄에 세울 수 없다는 것도 모름의 한 종류다. 열면 새로 온 사람이
레디스에 순번을 쥔 사람을 앞지른다. 신선한 스냅샷의 빈 줄도 예외가 아니다 — 한 노드만 레디스를 잃거나
레디스가 갓 죽으면 스냅샷이 낡음 문턱까지 신선으로 남아, 그 사이 다른 노드가 세운 줄을 모른다. 한산 몫 안의
통과는 레디스를 안 거쳐 그대로다. 대가는 레디스 장애 중 줄로 가는 신규가 전부 503 이라는 것이다.
...
```
 ...
 3. mode == ALWAYS && !queueFull      → ENQUEUE_ALWAYS       ← 낡음보다 앞. queueFull 은 6번과 같이 낡은 가득 포함
 ...
 6. waiting > 0 && waiting >= queueCapacity
    || dataStale && seenFull          → REJECT_QUEUE_FULL    ← 큐로 가는 경로보다 앞. 낡으면 노드가 본 가득 (CY-1006)
...
> 직후다. 큐가 안 닿아 등록이 실패하면 거절이다 (CY-1006) — 그 구간에 다른 노드가
> 방금 세운 줄이 스냅샷에 아직 없을 확률이 가장 높다.
...
> 요청 경로 레디스 왕복을 시도한다. 6번은 로컬에서 답을 내던 자리였다. CY-1006 전에는
> 그 왕복이 실패하면 fail-open 으로 받아 **꽉 찬 줄을 통째로 추월했다.**
>
...
12. **완료 (CY-1006)** **등록이 실패하면 어떤 줄 판정도 안 연다** — `REJECT_OVERLOAD`
