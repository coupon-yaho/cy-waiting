...
# 아무도 안 본다 — 실제로 이 저장소에서 게이트웨이를 아예 안 띄운 실행이 통과했고,
# 표본 0 건인 실행이 통과했다. 둘 다 초록이라 아무도 안 봤다.
#
...
# 통과하는 실행. 아래 사례들은 여기서 한 값씩만 비튼다 — 그래야 무엇이 잡혔는지가
# 값 하나로 좁혀진다.
base() {
  cat <<'JSON'
{
...
# 심판이 헐거워지는 방향
case_is "바닥이 시끄러우면 못 잰 실행으로 본다" 1 \
...
# 실행이 기구를 안 잰 경우는 통과도 미달도 아니다.
: > "$work/empty.tsv"
# 걷은 것도 걷었어야 하는 것도 없으면 기구에 기회가 없던 실행이다.
run_waste "기회가 없던 실행은 미판정" 1 "$work/s.log" "$work/empty.tsv" 1000 0 200 "$TTL"
...
# **못 되짚은 유령이 실행을 덮으면 미판정.** 알리바이 하나만 두고 나머지가 전부
# 표본 밖이면, 스위퍼가 한 명도 안 걷은 실행이 통과한다.
