...
#
# 크레딧 낭비는 **차례를 줬는데 아무도 안 받아 간 몫**이다. 실행이 끝나고 남은
# 사람들이 다 받아 간 뒤에도 입장 임계 **아래**에 남아 있는 줄 항목이 정확히
# 그것이다 — 그 사람은 차례를 받았고, 오지 않았다.
#
# **모델을 안 쓴다.** 이전 실행은 "걷을 수 있었던 인원" 을 시나리오 상수로
...
#   입장 임계 표본  = 실행 도중 1초마다 뜬 `admitted:{cid}`
#   남은 유령       = 회차가 끝난 뒤 임계 아래에 남은 줄 항목
