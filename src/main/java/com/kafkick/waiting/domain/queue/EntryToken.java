 ...
 *
 * <p><b>레디스를 안 친다</b> (RD-4). 검증에 조회가 필요하면 발급마다 왕복이 생기고,
 * 그 왕복이 곧 통과 인원에 비례한다.
 */
public final class EntryToken {

    /**
     * 입장 토큰의 수명. 발급 계층 명세가 정한 값이다.
     *
     * <p><b>짧아야 한다.</b> 차례가 온 사람이 쓰라고 준 것이지 나중에 쓰라고 준
     * 것이 아니다 — 길면 받아만 두고 나중에 몰려와 그 순간 상한을 넘긴다.
     */
    public static final long TTL_SEC = 180;

    /**
     * 발급 값을 끊는 단위. 짧을수록 폴링마다 토큰이 갈리고, 길수록 수명 편차가
     * 커진다. 최소 수명은 {@code TTL_SEC - WINDOW_SEC} 이다.
     */
    ...
    private EntryToken(byte[] secret) {
        this.secret = secret;
    }
    ...
    public static EntryToken of(String secret) {
        ...
        return new EntryToken(secret.getBytes(StandardCharsets.UTF_8));
    }

    /**
     * 발급한다. <b>같은 사람에게 늘 같은 값을 준다</b> — 폴링마다 갈리면 앞서
     * 받은 토큰이 조용히 죽고, 그 사람은 자기 차례를 못 쓴다.
     */
