...
식별자는 언어 문법과 도구가 함께 읽는다. 스택트레이스, 프로파일러 출력, 리플렉션
문자열, IDE 자동완성이 전부 식별자를 그대로 노출한다. 그보다 큰 것은 **일관성**이다
— 세 패키지 중 하나만 다른 규칙을 쓰면 그건 선호의 문제가 아니라 규칙이 없는 것과
같다. 실제로 한 패키지가 이걸 어겨 식별자 16개가 한글이었다.

**시험도 타입 이름은 영문이다.** `TS-2` 가 허용하는 것은 시험 *이름* 과 헬퍼이지
클래스·레코드명이 아니다. 경계는 파일이 아니라 **선언 종류**로 긋는다 — 파일 단위로
면제하면 그 안의 모든 선언이 함께 빠져 정작 막으려던 것이 샌다.

**운영 코드에 유니코드 이스케이프(`\uXXXX`)를 쓰지 않는다.** 자바는 렉싱
**전에** 이걸 풀어서 `\uD55C\uAE00` 은 한글 식별자가 되고 `/* \u002a/` 는 주석을
닫는다. 경계를 바꾸는 것만 골라내려 들면 목록이 끝없이 늘어난다.

### 검사는 파서에게 맡긴다

**직접 짜지 않는다.** 이 판정은 주석·문자열·텍스트 블록을 가려내고 유니코드
이스케이프까지 다뤄야 하는데, 그건 파서가 하는 일이다. 손으로 쓴 정규식은
어노테이션·수식어·줄바꿈이 조금만 달라도 새거나 반대로 무관한 것을 문다 —
실제로 그렇게 짰다가 우회 일곱과 오탐 둘을 겪고 갈아엎었다 (DS-8).

| 대상 | 도구 | 설정 |
|---|---|---|
| `src/main` | Checkstyle | `config/checkstyle/checkstyle.xml` |
| `src/test`·`testFixtures` | Checkstyle | `config/checkstyle/checkstyle-test.xml` (타입 이름만) |
| `*.gradle` | CodeNarc | `config/codenarc/codenarc.groovy` |

둘 다 Gradle 배포에 들어 있어 새 인프라가 없다. `./gradlew check` 가 함께 돈다.
