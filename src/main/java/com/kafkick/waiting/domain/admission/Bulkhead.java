...
/**
 * 동시에 몇 건이 뒷단에 걸려 있는지를 셉니다. <b>레이트 리밋으로는 못 막는 것이
 * 있습니다</b> — 초당 100건이어도 각각 10초가 걸리면 동시 1,000건이고, 느려진 뒷단
 * 한 대가 커넥션을 다 붙잡으면 한산한 쿠폰의 통과 경로까지 같이 죽습니다.
 */
public class Bulkhead {
    ...
    /**
     * 전체 합. <b>들고 다니지 않으면 읽을 때마다 맵을 훑습니다.</b> 그 훑기가 자리를
     * 잡는 것과 같은 자물쇠 안이라, 지표를 한 번 긁는 동안 요청 경로가 멈춥니다.
     */
    private int total;
    ...
    /**
     * 천장을 넘겨 쓸 수 있는 폭. <b>합의 상한이 이 값으로 정해집니다</b> — 한산한
    ...
    /**
     * 자리를 돌려줍니다. <b>안 돌려주면 격벽이 한 번 차고 나서 영영 안 열립니다</b> —
     * 성공·실패·취소 어느 쪽으로 끝나든 반드시 불러야 합니다. 안 들어간 것을
     * 내보내도 음수로 안 내려갑니다. 내려가면 그만큼 상한이 늘어납니다.
     */
