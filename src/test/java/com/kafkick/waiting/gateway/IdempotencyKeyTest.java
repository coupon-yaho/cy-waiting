 ...
 *
 * <p><b>게이트웨이가 끊어도 뒷단은 처리했을 수 있다.</b> 타임아웃은 응답을 안
 * 기다리겠다는 뜻이지 뒷단이 안 했다는 뜻이 아니다. 사용자가 다시 시도하면 같은
 * 사람이 두 번 발급된다 — 초과 발급이다 (불변식 2).
 *
 * <p>재사용 방지는 발급 계층의 멱등성이 진다 (A-10). 게이트웨이는 <b>같은 시도에
 * 같은 키</b>를 실어 그 멱등성이 작동할 근거를 준다.
 */
