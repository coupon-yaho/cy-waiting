...
**현재 상태**: 제로베이스 재작성 중. **Phase 9(Routing) — v0.6.0. 검증까지 끝냈고 게이트 하나가 미달이다.**
Phase 1~7 은 닫혔다. Phase 8 은 부분 충족으로 닫았고, Phase 9 는 검증(9.4) 다섯을 다 쟀다 — **라운드로빈으로는** 넷을 넘겼고, 인스턴스 종료 시 유출은 미달이며 P2C 는 하네스가 깊이를 못 만들어 아직 실측이 없다. 무엇이 남았는지는 각 게이트 표가 든다. 브랜치는 태스크 단위로 딴다 (WF-3).
