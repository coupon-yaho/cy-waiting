...
**봇이 연 PR 은 PR 워크플로를 통째로 건너뛴다.** 브랜치명 규칙도 커밋 규약도
작업 로그도 사람이 지킬 규칙이라 봇은 만족시킬 수 없다. 그대로 두면 의존성
갱신마다 빨간 체크가 쌓이고, **늘 빨간 것은 늘 초록인 것과 똑같이 아무 신호도
안 준다.**

가르는 기준은 브랜치 이름이 아니라 요청자 종류(`user.type`)다 — 이름 규칙은
봇마다 다르고 바뀐다.

> **맞바꾼 것.** 갱신된 의존성이 빌드를 깨는지 PR 에서 안 보게 된다.
> 그건 병합 뒤 `main` 과 `nightly` 가 잡는다.
