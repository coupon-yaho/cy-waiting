...
# **`*[!0-9.]*` 로는 부족하다.** `1..2` 나 `.` 도 통과하는데 `sleep` 이 그것을
# 못 읽는다. 루프는 실패한 `sleep` 을 무시하고 계속 도므로 레디스를 쉼 없이
# 친다 — 재려던 대상을 계기가 흔드는 상태가 되고, stderr 가 버려져 안 보인다.
if ! printf '%s' "$interval" | grep -qE '^[0-9]+(\.[0-9]+)?$' \
        || [ "$(awk -v v="$interval" 'BEGIN{print (v > 0) ? 1 : 0}')" != 1 ]; then
    echo "PROBE_INTERVAL_SEC 은 0 보다 큰 수여야 한다: '$interval'" >&2
    exit 2
fi
