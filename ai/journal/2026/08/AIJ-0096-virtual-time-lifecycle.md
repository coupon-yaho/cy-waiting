...
전부 빠졌다. 홀더의 시계도 고정값으로 못 박았다 — **가상 시계는 스케줄러만
가상화한다.**

팩토리가 터지면 `start()` 가 깃발을 되돌린다.
...
**팩토리가 새 실패 지점이다.** 안에서 만들 때는 터질 일이 없었는데, 밖에서 받으면
그럴 수 있다. 깃발을 먼저 세우고 나중에 만드는 순서라, 안 되돌리면 다음
`start()` 가 즉시 돌아간다 — 루프는 안 도는데 도는 것처럼 보이는 상태로 굳는다.
...
`스케줄러를_못_만들면_다시_시작할_수_있다` 가 팩토리 실패를 잰다. 한 번 터뜨린 뒤
정상으로 바꿔 다시 켜면 판이 돈다.
