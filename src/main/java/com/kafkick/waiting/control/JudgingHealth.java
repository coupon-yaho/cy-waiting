 ...
 * 동시 이탈로 번진다 — 낡은 재료로 판정하는 것보다 훨씬 나쁘다. 이름을 못 박은 것은
 * 기본 이름에 맡기면 클래스 이름을 고치는 것만으로 헬스 그룹에서 빠지기 때문이다.
 */
@Component("judging")
public final class JudgingHealth implements HealthIndicator {
    ...
    /**
     * 못 받는 경우는 둘뿐이다 — <b>드레이닝</b>과 <b>판정 재료 없음</b>. 받아오기
     * 실패도 재료가 낡은 것도 여기서 안 뺀다. 공유인지 국소인지 못 가리기 때문이고,
     * 못 가리면 안 빼는 것이 안전한 쪽이다.
     */
    @Override
    ...
    /**
     * 판정 재료가 있나. <b>루프가 돌았는가로 재면 안 된다</b> — 받아오기에 실패해도
     * 루프는 돈다. 그걸로 재면 레디스가 죽은 채 뜬 노드가 빈 스냅샷으로 받기 시작해
     * 전 쿠폰이 매진으로 보인다.
     */
