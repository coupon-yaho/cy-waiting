...
-- 것은 그 사람이 폴링할 때다. 걷으면 그가 다음 폴링에서 종료를 받아 다시 서게
-- 된다 (불변식 4).
...
if nFront > 0 then
    ...
    -- 회복 구간을 지키는 것은 SweepGate 의 재개 유예다. 창이 이미 임계 위라 아래의
    -- rank 검사는 대개 참인데 지우면 안 된다 — 범위 인자가 문자열을 거치며 임계가
    ...
    for i = 1, removing and nFront or 0 do
        ...
        -- 답해 줄 전체를 추월하고 초과 발급이 된다 (불변식 2·4).

        -- **임계 위의 `a:` 는 낡은 값임이 증명된다.** 지금 차례가 온 사람은 임계 아래라
        -- 창에 없다. 건너뛰면 그 사이 임계가 그를 지나가 창 밖이 되어 영영 안 걷히고,
        -- 줄 길이가 영구히 부풀어 그 쿠폰이 한산으로 안 돌아간다 (R1 이 꺼진다).
